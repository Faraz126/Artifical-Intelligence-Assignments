before_in_year(X, Y, Months) :-	nth1(Xi, Months, X),nth1(Yi, Months, Y),Xi < Yi.
print_solution([Head|Tail]) :-	write(Head),nl,print_solution(Tail).

solution(S) :-
	S = [[abigail, Month1, Day1],
	     [brenda, Month2, Day2],
	     [mary, Month3, Day3],
	     [paula, Month4, Day4],
	     [tara, Month5, Day5]],
...
%	Girls = [Name1, Name2, Name3, Name4, Name5],
	...
	% and each on a different day of the week.
	Days =	[saturday,monday,wednesday,friday, sunday],
	...
	member([brenda, Month2, _], S), before_in_year(C1, Month2, Months),
